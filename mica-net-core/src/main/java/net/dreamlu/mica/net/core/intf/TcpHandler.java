...
/**
 * TCP 业务处理器，会话上下文固定为 {@link ChannelContext}。
 * <p>
 * 用法：
 * <pre>{@code
 * public class MyHandler implements TcpHandler {
 *     public Packet decode(ByteBuffer buf, int limit, int pos, int readable, ChannelContext ctx) { ... }
 *     public ByteBuffer encode(Packet p, ChannelContext ctx) { ... }
 *     public void handler(Packet p, ChannelContext ctx) { ... }
 * }
 * }</pre>
 *
 * @author L.cm
 */
public interface TcpHandler extends TioHandler<TioConfig, ChannelContext> {
