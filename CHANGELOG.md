...
- fix(timer): `DefaultTimerTaskService` 默认时间轮粒度由 `(tickMs=1000, wheelSize=60)` 调整为 `(tickMs=10, wheelSize=100)`，并抽出常量 `DEFAULT_TICK_MS` / `DEFAULT_WHEEL_SIZE`。`TimingWheel.add` 对「过期时刻 < currentTime + tickMs」的任务直接返回 false，`SystemTimer` 收到 false 会**立即提交执行**，因此任何 `delayMs < tickMs` 的任务延时都会退化为 0；旧默认值 1000ms 的粒度会让所有亚秒级延时变成「立即执行」（实测 delay=100ms 触发于 0ms），客户端 `reInterval(100)` 的重连间隔随之退化成 0 并形成重连风暴（实测 1 秒内重连 500+ 次，修复后为 ~10 次）。`TioClient` / `TioServer` / `TimedCache` 三处默认取值一并受益。
- docs(timer): `TioClient#getTimerTaskService` 补注释指向 `DefaultTimerTaskService#DEFAULT_TICK_MS`，避免业务层再硬编码一套粒度。
...
- feat(mcp): `SseTransport` 默认端点由 `/sse` 调整为 `/mcp/sse`（message 端点同步调整为 `/mcp/sse/message`），与 `StreamableHttpTransport` 默认的 `/mcp` 保持同一根路径，避免同时启用两个 transport 时路由冲突。`/mcp` 归属 modern Streamable HTTP，`/mcp/sse` 归属 legacy SSE，二者路径不重叠。注意：此变更向后不兼容，依赖默认端点的用户需在 `McpServer#useSseTransport` 显式传旧值或升级客户端配置。
- test(mcp): 补充 5 个 MRTR + JSON Schema 2020-12 单元测试，覆盖 modern input_required、legacy fallback、deprecated 警告、$defs / $ref / 组合关键字 / 序列化往返。
- test(mcp): 补充 11 个 modern 协议单元测试覆盖 `server/discover`、`resultType` 注入、列表缓存字段、确定性顺序、`MCP_2026_07_28` 注册等场景。
- test(mcp): 新增 `McpTransportTest` 覆盖 SSE/Streamable 默认端点常量及互不冲突断言。
...
- refactor(core): 新增 TcpHandler 接口增强类型安全性
